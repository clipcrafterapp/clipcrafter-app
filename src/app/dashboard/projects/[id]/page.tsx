...
export function ProjectDetailContent({ id }: { id: string }) {
  ...
  const [clipsStatus, setClipsStatus] = useState<
    "idle" | "generating" | "done" | "failed" | string
  >("idle");
  ...
  function handleHandleMouseDown(e: React.MouseEvent, clipId: string, side: "start" | "end") {
    ...
    function onMouseUp() {
      ...
      setClips((prev) => {
        ...
        if (clip) {
          const update =
            side === "start" ? { start_sec: clip.start_sec } : { end_sec: clip.end_sec };
          fetch(`/api/clips/${cId}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(update),
  ...
  const isProcessing =
    data && !TERMINAL_STATUSES.includes(data.status) && data.status !== "pending";
  ...
  const captionSegment =
    showCaptions && data?.transcript?.segments
                ...
                {isCompleted && (
                  <>
                    ...
                    {viewMode === "graph" && computedGraph && (
                      <>
                        ...
                        <div className="flex flex-col gap-3 bg-gray-900 border border-gray-800 rounded-xl p-4">
                          ...
                          {(sortedClips ?? []).map((clip) => {
                            ...
                            const node = computedGraph.nodes.find(
                              (n) =>
                                computedGraph.segments.find((s) => s.id === clip.id)?.topicId ===
                                n.id
                            );
                            return (
                              <div
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2 ml-5">
                                  <label className="text-xs text-gray-500 w-8 shrink-0">
                                    Start
                                  </label>
                                  <input
                                    type="number"
                        ...
                        {selectedClipIds.size > 0 && (
                          ...
                          >
                            Export {selectedClipIds.size} clip
                    ...
                    {viewMode === "list" &&
                      (() => {
                        const topics = [
                          ...new Set((clips ?? []).map((c) => c.topic).filter(Boolean) as string[]),
                        ];
                        ...
                        <div className="sticky top-0 z-10 bg-gray-950 py-2 flex items-center gap-3 border-b border-gray-800 -mx-4 px-4">
                            ...
                            <input
                              ...
                              ref={(el) => {
                                if (el)
                                  el.indeterminate =
                                    selectedClipIds.size > 0 &&
                                    selectedClipIds.size < sortedClips.length;
                              }}
                              onChange={(e) => {
                                if (e.target.checked) {
                          ...
                          <button
                            ...
                            disabled={
                              selectedClipIds.size === 0 ||
                              (clips?.some(
                                (c) => selectedClipIds.has(c.id) && c.status === "exporting"
                              ) ??
                                false)
                            }
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-3 mb-2 text-xs text-gray-400 font-mono">
                                    ...
                                    <span className="bg-gray-800 px-1.5 py-0.5 rounded text-gray-300">
                                      {clip.duration_sec?.toFixed(1) ??
                                        (clip.end_sec - clip.start_sec).toFixed(1)}
                                      s
                                    </span>
                                  </div>
                                  ...
                                  {visibleTags.length > 0 && (
                                    <div className="flex flex-wrap gap-1 mb-3">
                                      {visibleTags.map((tag) => (
                                        <span
                                          key={tag}
                                          className="text-xs bg-gray-800 text-violet-400 px-2 py-0.5 rounded-full"
                                        >
                                          {tag}
                                        </span>
                                      ))}
                                  ...
                                  >
                                    <button
                                      ...
                                      onClick={() =>
                                        handleClipAction(clip.id, {
                                          status:
                                            clip.status === "approved" ? "pending" : "approved",
                                        })
                                      }
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        handleClipAction(clip.id, {
                                          status:
                                            clip.status === "rejected" ? "pending" : "rejected",
                                        })
                                      }
                                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors min-h-[44px] ${
                                        clip.status === "rejected"
                                    ...
                                    <select
                                      ...
                                      onChange={(e) =>
                                        handleClipAction(clip.id, { aspect_ratio: e.target.value })
                                      }
                                      className="bg-gray-800 border border-gray-700 text-gray-300 text-xs rounded-lg px-2 py-1.5 min-h-[44px]"
                                    >
                          ...
                          <div className="flex flex-col gap-2 pt-1">
                            {(data.transcript.segments as Segment[]).map((seg) => {
                              ...
                              const text = speakerMatch
                                ? seg.text.slice(speakerMatch[0].length)
                                : seg.text;
                              const speakerColors = [
                                "text-violet-400",
                                "text-blue-400",
                              ...
                              return (
                                <div key={seg.id} className="flex gap-3 text-sm">
                                  ...
                                  {speakerNum !== null && (
                                    <span
                                      className={`text-xs font-bold shrink-0 pt-0.5 w-16 ${color}`}
                                    >
                                      S{speakerNum}
                                    </span>
                                  )}
                          ...
                          {data.processing_log.map((entry, i) => (
                            <div key={i} className="flex items-start gap-3 text-sm">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {entry.provider && (
                                  <span className="ml-2 text-violet-400 text-xs font-mono">
                                    {entry.provider}
                                  </span>
                                )}
                                {entry.detail && (
        ...
        <main className="flex-1 flex flex-col min-w-0 lg:sticky lg:top-0 lg:h-screen">
          {!isCompleted ? (
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              <div
                className={`shrink-0 bg-gray-900 border-t border-gray-800 px-4 py-3 flex flex-col gap-2 ${isYouTube ? "hidden" : ""}`}
              >
                {/* Row 1 */}
                <div className="flex items-center gap-3">
                  {/* Skip prev */}
