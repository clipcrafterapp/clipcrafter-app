...
type RowProps = { project: AdminProject; deleting: boolean; onDelete: (id: string, title: string) => void };
  ...
  return (
    <tr className="border-b border-gray-800 hover:bg-gray-800/40">
      ...
      <td className="px-4 py-3">
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[project.status] ?? "bg-gray-700 text-gray-300"}`}>
          {project.status}
        </span>
      </td>
      <td className="px-4 py-3 text-gray-400 text-xs">{formatDate(project.created_at)}</td>
      <td className="px-4 py-3">
        <button type="button" disabled={deleting} onClick={() => onDelete(project.id, project.title)}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">Loading…</td></tr>
              : projects.length === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">No projects found</td></tr>
              : projects.map((p) => <ProjectRow key={p.id} project={p} deleting={deleting === p.id} onDelete={(id, title) => void handleForceDelete(id, title)} />)}
          </tbody>
        </table>
      </div>
      {totalPages > 1 && (
